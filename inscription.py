...
class new_win(QtWidgets.QWidget):
   ...
   #    info = (nbr_seance,1,current_date,id_client,id_cours)
   #    db.cursor().execute("UPDATE inscription set nombre_seances = %s,Absences_Tolérées=%s,Date_inscription = %s where id_client = %s and id_cours= %s",info)
   #    db.commit()
   def update_inscri_clicked(self):
      global db,nbr_seance
      cur = db.cursor()
      ...
      nbr_seance = int(nbr_seance) + int(nbr_scie_exist)
      current_date=date.today()
      info = (nbr_seance,1,current_date,id_client,id_cours,de_test)
      ...
      db.cursor().execute("UPDATE inscription set nombre_seances = %s,Absences_Tolérées=%s,Date_inscription = %s where id_client = %s and id_cours= %s, de_test = %s",info)
      ...
      for i in groupes_selected:
         stri += "," + str(i)
      mssg.setText("l'inscription est reussite et voila votre groupes :" + stri)
   ...
   def submit_nbr_seance(self):
      global niv,id_group,libele,service,de_test,nbr_seance,id_client,done
      curs = db.cursor()
